...
class Rotary(nn.Module):
    ...
    def __init__(self, dim: int, max_seq_len: int):
        ...
        steps = max(1, dim//4)  # Ensure at least 1 step
        angular_freq = (1 / 10000) ** torch.linspace(0, 1, steps=steps, dtype=torch.float32)
        
        # Mirror the frequencies (mathematical requirement)
        mirror_size = max(1, dim//4)
        angular_freq = torch.cat([angular_freq, angular_freq.new_zeros(mirror_size)])
